...
export default function ExtendMembershipScreen() {
  const router = useRouter();
  const { user, setUser } = useStore();
  const [selectedTierIndex, setSelectedTierIndex] = useState<number>(1);

  const handlePurchase = () => {
    // Perform payment and update status in the store
    const tier = TIERS[selectedTierIndex];
    let expiryDate = "31 Juli 2025";
    if (tier.name === "Bulanan") {
      expiryDate = "30 Juni 2026";
    } else if (tier.name === "Semesteran") {
      expiryDate = "30 November 2026";
    } else if (tier.name === "Tahunan") {
      expiryDate = "25 Mei 2027";
    }
    ...
    alert(`Perpanjangan paket ${tier.name} berhasil!`);
    router.back();
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Perpanjang</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        >
          <Text style={styles.primaryButtonText}>
            {selectedTierIndex !== null ? `Bayar & Perpanjang ${TIERS[selectedTierIndex].name}` : "Pilih Paket Terlebih Dahulu"}
          </Text>
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
